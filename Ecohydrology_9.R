...
library(stats)
setwd("/Volumes/GoogleDrive/My Drive/Cornell/Ecohydrology/Week9")
...
# asssume a forest cover value, but is it okay that this is calibrated for present day? With CC we expect there 
# to a change in forest make up   
# assumes that plant comp stays the same 
# assumes parameters do not change (calibrated to historical conditions, these are specific to a historic period)
# assumes people do not plow and redistribute snow
# bigassumptions that we do not change land use and population (prob biggest assumption)

GCM_SWE_Max = data.frame(matrix(nrow = (2100 - 2015), ncol = 0))   
# populate the SWE_Max df with 5 columns of maximum annual values
# Aggregate function is used to find the maximum value of SWE in each year. This is 
# repeated for each model. 
GCM_SWE_Max$Access = (aggregate(snowmelt_Access$SnowWaterEq_mm, by = list(year(snowmelt_Access$Date)), max)[,-1])
GCM_SWE_Max$bcc = (aggregate(snowmelt_bcc$SnowWaterEq_mm, by = list(year(snowmelt_Access$Date)), max)[,-1])
GCM_SWE_Max$BNU = (aggregate(snowmelt_BNU$SnowWaterEq_mm, by = list(year(snowmelt_Access$Date)), max)[,-1])
GCM_SWE_Max$CanESM = (aggregate(snowmelt_CanESM$SnowWaterEq_mm, by = list(year(snowmelt_Access$Date)), max)[,-1])
GCM_SWE_Max$GFDL = (aggregate(snowmelt_GFDL$SnowWaterEq_mm, by = list(year(snowmelt_Access$Date)), max)[,-1])
GCM_SWE_Max$mean = rowMeans(GCM_SWE_Max)
GCM_SWE_Max$Year = seq(2015, 2099, by = 1)
...
# - what are the physical / ecological processes that are driving this change?
# - note any limitations or assumptions in our the models that might bias these results
GCM_Q_Max = data.frame(matrix(nrow = (2100 - 2015), ncol = 0))   
...
GCM_Q_Max$Access = (aggregate(Results_Access$modeled_flow, by = list(year(Results_Access$Date)), max)[,-1])
GCM_Q_Max$bcc = (aggregate(Results_bcc$modeled_flow, by = list(year(Results_Access$Date)), max)[,-1])
GCM_Q_Max$BNU = (aggregate(Results_BNU$modeled_flow, by = list(year(Results_Access$Date)), max)[,-1])
GCM_Q_Max$CanESM = (aggregate(Results_CanESM$modeled_flow, by = list(year(Results_Access$Date)), max)[,-1])
GCM_Q_Max$GFDL = (aggregate(Results_GFDL$modeled_flow, by = list(year(Results_Access$Date)), max)[,-1])
GCM_Q_Max$mean = rowMeans(GCM_Q_Max)
GCM_Q_Max$Year = seq(2015, 2099, by = 1)  
