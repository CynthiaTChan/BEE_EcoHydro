...
setwd("/Volumes/GoogleDrive/My Drive/Cornell/Ecohydrology/Week10")
...
MetData <- MetData[which(MetData$Year > 1963 & MetData$Year < 1966),]
FC <- get_usgs_gage(flowgage_id = "04234000", begin_date = "1964-01-01", end_date="1966-12-31")
...
p1 <- ggplot(Results, aes(x = Date, y = modeled_flow)) +
  geom_line(aes(y = modeled_flow)) + 
  labs(title = "1994 - 1995: Modeled Discharge and Discharge Flow in Fall Creek", 
       x = "Date, y = Discharge")
